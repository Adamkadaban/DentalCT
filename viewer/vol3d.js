...
export async function initVolume(canvas, statusEl){
  ...
  uniform float yaw,pit,dist,panx,pany,aspect,thr,mode;
  mat3 rot(float y,float p){float cy=cos(y),sy=sin(y),cx=cos(p),sx=sin(p);
  ...
  void main(){vec3 L=normalize(vec3(.4,.6,.8)); mat3 R=rot(yaw,pit); vec3 pan=vec3(panx,pany,0);
  ...
  let yaw=0,pit=-1.4,dist=2.4,px2=0,py2=0,thr=.35,mode=0;
  function draw(){const w=canvas.clientWidth||300,h=canvas.clientHeight||300;canvas.width=w;canvas.height=h;gl.viewport(0,0,w,h);
    gl.uniform1f(U("yaw"),yaw);gl.uniform1f(U("pit"),pit);gl.uniform1f(U("dist"),dist);gl.uniform1f(U("panx"),px2);gl.uniform1f(U("pany"),py2);
    gl.uniform1f(U("aspect"),w/h);gl.uniform1f(U("thr"),thr);gl.uniform1f(U("mode"),mode);gl.drawArrays(gl.TRIANGLES,0,3);}
  let dn=0,mx0,my0;
  ...
  window.addEventListener("mousemove",e=>{if(!dn)return;const dx=e.clientX-mx0,dy=e.clientY-my0;mx0=e.clientX;my0=e.clientY;
    if(dn&1){yaw-=dx*.01;pit=Math.max(-1.55,Math.min(1.55,pit-dy*.01));}else{px2-=dx*.003*dist;py2+=dy*.003*dist;}draw();});
  canvas.onwheel=e=>{dist*=Math.exp(e.deltaY*.001);e.preventDefault();draw();};
  statusEl.textContent="drag·wheel·right-pan"; draw();
